...
async fn index_p(Path(path): Path<PathBuf>,ConnectInfo(addr): ConnectInfo<SocketAddr>,headers: HeaderMap,Extension(conf): Extension<Conf>,request: axum::http::Request<axum::body::Body>) -> impl IntoResponse {
    ...
    let mut req = client.request(
        method.clone(),
        format!("{}/{}", conf.url, path.to_string_lossy()),
    );

    if matches!(method, reqwest::Method::POST | reqwest::Method::PUT | reqwest::Method::PATCH | reqwest::Method::DELETE) {
...
async fn index(ConnectInfo(addr): ConnectInfo<SocketAddr>,headers: HeaderMap,Extension(conf): Extension<Conf>,request: axum::http::Request<axum::body::Body>) -> impl IntoResponse {
    let client = CLIENT.clone();
